		...
		<>
			<Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
				<div className="relative h-64 ">
					...
					>
						<Heart className={`h-4 w-4 ${isFavoriteHero(hero) ? " fill-red-600 text-red-600" : " fill-gray-600 text-gray-600"}`} />
					</Button>
				...
				<CardHeader className="py-3 z-10 bg-gray-100/50 backdrop-blur-sm relative top-1 group-hover:top-[-10px] transition-all duration-300">
					<div className="flex justify-between items-start">
						<div className="space-y-1">
							<h3 className="font-bold text-lg leading-tight text-red-500">
								{hero.alias}
							</h3>
				...
				<CardContent className="space-y-4 mt-4">
					...
					<div className="space-y-2">
						...
						<div className="flex flex-wrap gap-1">
							{/* No queremos mostrar infinidad de habilidades */}
							...
							{hero.powers.length > 3 && (
								<Badge className="text-xs">+{hero.powers.length - 3}</Badge>
							)}
						</div>
					</div>
